...
use App\PaymentChannels\Drivers\Posfix\Channel;
...
class PaymentController extends Controller
{
    ...
    public function posfixCallback(Request $request, $id)
    {
        // Callback'den gelen verileri al
        $data = $request->all();

        try {
            // Order ID üzerinden siparişi bulun
            ...
            if ($data['mdStatus'] == '1') {
                ...
                return view('failure', ['error' => 'Ödeme doğrulaması başarısız.']);
            }
        } catch (\Exception $exception) {
            // Hata durumunda işlem
            return view('failure', ['error' => $exception->getMessage()]);
        }
    }
...
public function paymentVerify(Request $request, $gateway)
{
    ...
    try {
        ...
        $toastData = [
            'title' => "Hata Detayı",
            'msg' => $exception->getMessage(), // Hata mesajını gösterelim
            'status' => 'error'
        ];
        
        \Log::error('Payment Verify Error: ' . $exception->getMessage()); // Loglara kaydedelim
        
        return redirect('cart')->with(['toast' => $toastData]);
    }
}
   public function posfixProcess(Request $request)
    {
        // Sipariş ve kullanıcı bilgilerini al
        $order = Order::findOrFail($request->order_id);
        $user = $order->user;

        // Ödeme kanalını al ve verileri gönder
        $paymentChannel = PaymentChannel::where('class_name', 'Posfix')->first();
        $channel = new Channel($paymentChannel);

        // Verileri channel'a ilet
        $response = $channel->send3DRequest($request);

        if ($response) {
            // Formu ve preparedData'yı ekrana yazdırma işlemi
            $channel->printForm($response['form'], $request->all());
        }

        return redirect()->back()->with('msg', trans('cart.payment_failed'));
    }
